...
class PlayState extends MusicBeatState
{
	...
	public static var dadAbsis:Int = 0;
	public static var dadOrdinata:Int = 0;

	public static var bfAbsis:Int = 0;
	public static var bfOrdinata:Int = 0;
	...
	override public function create()
	{
		...
		switch (curStage)
		{
			...
			case 'kitchen': //Among Us Week
				var bg:BGSprite = new BGSprite('bgs/kitchenbg', -350, -125, 0.9, 0.9);
				bg.setGraphicSize(Std.int(bg.width * 0.8));
				bg.updateHitbox();
				add(bg);
		}
	...
	function moveCameraSection(?id:Int = 0):Void {
		...
		if (!SONG.notes[id].mustHitSection)
		{
			...
			camFollow.x += dadAbsis;
			camFollow.y += dadOrdinata;
		}
		else
		{
			moveCamera(false);
					...
					if(!boyfriend.stunned)
					{
						...
						if(!endingSong)
						{
							...
							if(!note.isSustainNote) {
								health -= 1; //0.26 + 0.04 = -0.3 (-15%) of HP if you hit a hurt note
								if(!note.noteSplashDisabled) {
									spawnNoteSplashOnNote(note);
								}
							}
							else health -= 1; //0.06 + 0.04 = -0.1 (-5%) of HP if you hit a hurt sustain note
	...
	function bgSabotageLight():Void
		{
			camGame.flash(FlxColor.fromRGB(179, 0, 0, 102), 0.35);
		}
